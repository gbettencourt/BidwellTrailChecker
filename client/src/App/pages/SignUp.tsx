...
import { FormControlLabel, Switch, Avatar, Box, Button, Container, CssBaseline, Grid, Link, TextField, Typography } from '@mui/material';
...
export default class SignUp extends Component<SignUpProps, SignUpState> {
	...
	handleRegisterClick() {
		const isValidEmail = (document.getElementById('email') as any).validity?.valid;
		if (!isValidEmail) {
			this.setState({
				isEmailValid: false,
		...
		const isValidPhone = this.state.sendSms ? phoneInput.validity?.valid && cleanPhoneNumber(phoneInput.value).length === 10 : true;
		...
		const isValidCaptcha = this.verifyCaptcha((document.getElementById('captcha') as HTMLInputElement).value);
		if (!isValidCaptcha) {
			this.setState({
								...
								<Box component="form" noValidate sx={{ mt: 3 }}>
									<Grid container spacing={2}>
										...
										<Grid item xs={12}>
											<FormControlLabel
												control={
													<Switch checked={this.state.sendSms} onChange={this.handleSendSmsChange} name="jason" />
												}
